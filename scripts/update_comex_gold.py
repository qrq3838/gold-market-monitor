#!/usr/bin/env python
"""Download and normalize Yahoo Finance daily COMEX gold futures history."""
...
SYMBOL = "GC=F"
SOURCE_PAGE = "https://finance.yahoo.com/quote/GC%3DF/history/"
API_HOSTS = ("query1.finance.yahoo.com", "query2.finance.yahoo.com")
CANONICAL_API_URL = "https://query1.finance.yahoo.com/v8/finance/chart/GC%3DF"
...
def fetch_yahoo() -> tuple[dict[str, Any], str]:
    ...
    for host in API_HOSTS:
        ...
        try:
            ...
            return payload, url
        except Exception as error:  # try the second Yahoo chart host
            errors.append(f"{host}: {error}")
    raise RuntimeError("Yahoo Finance download failed: " + "; ".join(errors))


def normalize(source: dict[str, Any]) -> tuple[list[dict[str, float | int | str]], dict[str, Any]]:
    ...
    fields = {name: quote.get(name) for name in ("open", "high", "low", "close", "volume")}
    ...
    by_date: dict[str, dict[str, float | int | str]] = {}
    for index, timestamp in enumerate(timestamps):
        ...
        by_date[day] = {
            ...
            **{
            ...
            "volume": int(values["volume"]) if values["volume"] is not None else 0,
        }
    ...
    for row in rows:
        prices = [float(row[name]) for name in ("open", "high", "low", "close", "adjusted_close")]
        if any(not 100 < price < 20000 for price in prices):
            raise ValueError(f"Implausible COMEX gold price on {row['date']}: {prices}")
        if int(row["volume"]) < 0:
            raise ValueError(f"Negative volume on {row['date']}")
    return rows, meta


def render_csv(rows: list[dict[str, float | int | str]]) -> str:
...
def main() -> int:
    ...
    json_path = args.output_dir / "comex_gold_futures.json"
    csv_path = args.output_dir / "comex_gold_futures.csv"
    ...
    print(
        f"{SYMBOL} through {dates[-1]}: US${values[-1]:,.2f}/oz "
