...
1. `assets/gold-overlay.js` 只载入一次 `data/comex_gold_futures.json`，供央行购金、ETF 资金流、ETF 持仓、实际利率和美元指数五张指标图共同使用。
...
5. 五张指标图默认显示 COMEX 金价；用户可通过图例按钮隐藏或重新显示曲线，隐藏时右轴和金价提示同时消失。独立 COMEX 金价图本身不重复叠加同一序列。
...
- 统一金价叠加重点巡检五张指标图是否都显示 `COMEX 金价` 按钮、右轴是否为 `US$/oz`，以及关闭按钮后曲线、右轴和提示是否同时消失。
