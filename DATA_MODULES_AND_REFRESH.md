...
3. 剔除 Close 为空的缺失占位，并识别 Yahoo 在日频接口末尾附加的盘中实时报价：如果该行时间戳等于 `regularMarketTime` 且未对齐交易所当地零点，则视为未完成日线并剔除。其余数据不做插值；有效日期必须唯一、严格递增，价格必须为正且位于预设合理区间，成交量不得为负。
...
3. 剔除 Close 为空的周末或缺失占位，并按交易所时区剔除 Yahoo 末尾时间戳未对齐当地零点的盘中实时报价，避免把盘中快照误作日收盘；其余数据不做插值，有效日期必须唯一、严格递增。
